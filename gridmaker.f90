...
!                               gridmaker                                       !
!                                                                               !
! gridmaker is a module that generates the Becke integration grid for a given   !
! molecule. You must specify values for stepf_iter, n_leb, radfac before calling!
! the routine.  The grid and the number of grid points are determined by the    !
! program.                                                                      !
!                                                                               !
!  Becke(n_atom,Ratom,Zatom)                                                    !
!                                                                               !
! input_grid -- reads the file parameters.grd that contains the parameters      !
!               defining a Becke-style grid.                                    !
! Becke -- gives the Becke integration grids.                                   !
!-------------------------------------------------------------------------------!
...
!                               input_grid                                      !
!                                                                               !
! This is a subroutine from Paul Ayers that reads the parameters.grd file.      !
! It is an adaptation of a routine by Patrick Bultinck                          !
!-------------------------------------------------------------------------------!
...
! open the file containing the parameters defining the Becke grid.
open (UNIT=10,FILE='parameters.grd',STATUS='old',IOSTAT=istat)

!Set defaults
stepf_iter = 3   !controls smoothing of atomic boundary
n_leb = 170      !number of angular points in the Lebedev grid.
radfac = 1.0D0   !Fraction of the recommended number of radial points.  
                 !Bigger numbers mean more points, more accurate grids, and
                 !slower programs.
radscale = 1.0D0 !No scaling of atomic radii to a first approximation.

DO
    read (10,'(A80)',IOSTAT=istat) line

    IF (line(1:12).EQ.'stepfunction') then
       READ(line,'(26X,I10)') stepf_iter
    ENDIF

    IF (line(1:4).EQ.'grid') then
       READ(line,'(7X,I6)') n_leb
    ENDIF
    
    IF (line(1:6).EQ.'radfac') then
       READ(line,'(9X,F30.10)') radfac
    ENDIF
...
IF (debug) THEN
   WRITE(*,*) 'smoothing iterations for Voronoi cells ',stepf_iter
   WRITE(*,*) 'number of angular points ',n_leb
   WRITE(*,*) 'radial factor ',radfac
ENDIF
...
! XYZ(1:3,1:n_grid) -- the grid points.                                         !
! W(1:n_grid) -- the grid weights.                                              !
! W_leb(1:n_leb) -- the angular weights.                                        !
! X_leb(1:n_leb) -- the angular points in the Lebedev grid, on the unit sphere. !
! Y_leb(1:n_leb) -- the angular points in the Lebedev grid, on the unit sphere. !
! Z_leb(1:n_leb) -- the angular points in the Lebedev grid, on the unit sphere. !
! BraggSlater_radii(1:95) -- the Bragg-Slater radii used to specify the         !
!                            relative position of the radial shells.            !
! n_radpts(1:118) -- the number of points in the radial grid for each atom.     !
! radpts -- the number of radial points in the grid. At first this is the total !
!           number of radial points in the grid; later it represents the number !
!           of points in a given atom.                                          !
! Wrad(1:nradpts) -- the weights for radial integration.                        !
! Rpts(1:nradpts) -- the radial points.                                         !
! gridcount -- counts the number of points in the grid.                         !
! natomRpts -- the number of radial points in a given atom.                     !
! atomic_radius -- the atomic_radius used for making the radial grid.           !
! istat -- status of the deallocation.                                          !
!-------------------------------------------------------------------------------!

subroutine Becke(n_atoms,Ratom,Zatom)

USE kinds; USE grid_vars

IMPLICIT NONE

INTEGER(istd) :: n_atoms,Zatom(1:n_atoms),n_radpts(1:118),radpts,natomRpts,i,j,k
INTEGER(istd) :: istat
REAL(dbl)     :: Ratom(1:3,1:n_atoms)
REAL(dbl)     :: b_weight
REAL(dbl)     :: w_leb(1:n_leb),X_leb(1:n_leb),y_leb(1:n_leb),Z_leb(1:n_leb)
REAL(dbl),ALLOCATABLE   :: Wrad(:),Rpts(:)
REAL(dbl)     :: BraggSlater_radii(1:95)
REAL(dbl)     :: atomic_radius

!Get the atomic radii.
CALL BraggSlater(BraggSlater_radii)

! Establish the number of radial points per atom according to Becke
call nr_radial_points(n_radpts,radfac)

DEALLOCATE(Wbecke,XYZbecke,stat=istat)

!count the total number of radial points
radpts = SUM(n_radpts(Zatom(1:n_atoms)))

!The total number of grid points is then
n_grid = radpts*n_leb
WRITE(*,*) 'Constructing molecular grid of',n_grid,' points...'
ALLOCATE(Wbecke(1:n_grid),XYZbecke(1:3,1:n_grid))

! Get the Lebedev grids according to the one chosen
! (THANK YOU Dr. Christoph van Wuellen)
call lebedev(x_leb,y_leb,z_leb,w_leb,n_leb)

! Start making the grid. We will store for the grid points the coordinates (3),
! the product of all the grid specific weights; NOT the Hirshfeld
! weight; and the density. We follow closely the scheme in the Becke paper.
n_grid = 0

do i=1,n_atoms
   !Get the number of radial points in this atom and allocate arrays for the
   !radial grid.
   radpts = n_radpts(Zatom(i))
   ALLOCATE(Wrad(1:radpts),Rpts(1:radpts))
   atomic_radius = radscale*BraggSlater_radii(Zatom(i))
   !Get the radial points.  This is based on Gauss-Chebyshev, after Becke.
   call becke_grid(atomic_radius,radpts,Wrad,Rpts)
 

   DO j=1,radpts
      DO k = 1,n_leb
         !Put the point onto the unit sphere.  This requires translating the center
         !of the coordinate system to the atomic position and scaling the sphere.
         n_grid = n_grid + 1
         XYZbecke(1,n_grid) = x_leb(k)*Rpts(j)+Ratom(1,i)
         XYZbecke(2,n_grid) = y_leb(k)*Rpts(j)+Ratom(2,i)         
         XYZbecke(3,n_grid) = z_leb(k)*Rpts(j)+Ratom(3,i)         

         !The points overlap, so we need to reweight the points so that
         !points in the interstitial regions do not get double-counted
         call becke_weight(Ratom,Zatom,n_atoms,BraggSlater_radii,XYZbecke(1:3,n_grid),&
                           i,b_weight)
         !The final integration weight is
         Wbecke(n_grid) = 4*3.14159265*Wrad(j)*W_leb(k)*b_weight
      ENDDO
   ENDDO
   DEALLOCATE(Wrad,Rpts)
ENDDO
...
!*******************************************************************************!
!-------------------------------------------------------------------------------!
! The following subroutines are taken from Patrick Bultinck and only very slightly
! modified by Paul Ayers.  I do not consider them "user modifiable."            
!-------------------------------------------------------------------------------!


subroutine nr_radial_points(nr_points,radfac)

! Establish the number of radial points for each atom
! Becke recipe
! radfac is the multiple of the number of "standard" Becke points that is
! being used.
...
INTEGER(istd) :: i
INTEGER(istd) :: nr_points(1:118)
REAL(dbl) :: radfac

FORALL(i=1:2)
      nr_points(i) = int(30*radfac)
ENDFORALL
FORALL(i=3:10)
      nr_points(i) = int(35*radfac)
ENDFORALL
FORALL(i=11:18)
        nr_points(i) = int(40*radfac)
ENDFORALL
FORALL(i=19:36)
        nr_points(i) = int(45*radfac)
ENDFORALL
FORALL(i=37:54)
        nr_points(i) = int(50*radfac)
ENDFORALL
FORALL(i=55:86)
        nr_points(i) = int(55*radfac)
ENDFORALL
FORALL(i=86:118)
        nr_points(i) = int(60*radfac)
ENDFORALL

end subroutine

subroutine becke_grid(Radii,nr_pts,radial_weights,radial_points)

! Subroutine for the Gauss-Chebyshev grid (2nd kind)
! with the integration interval mapped from 0 to infinite
! x_point is the coordinates of the point in (-1,1).
!            ==cos(i*pi/(n+1))
! radii is the Bragg-Slater radius of the atom.
! nr_pts is the number of radial points.
! radial_weights are the radial weights.
! radial_points are the radial points.

USE constants

implicit none

INTEGER::atom,nr_pts,j
REAL*8::x_point(1:nr_pts),radii
REAL*8,DIMENSION(nr_pts)::radial_weights,radial_points

FORALL(j=1:nr_pts)
   x_point(j) = COS(j*pi/(1.0+nr_pts))
ENDFORALL

radial_points(:) = (1.0+x_point(:))/(1.0-x_point(:))*radii/2.0
radial_weights(:) = 2.0*pi/(1.0+nr_pts)*(1.0+x_point(:))**2.5D0             &
                               /(1.0-x_point(:))**3.5D0 * (radii/2.0)**3.0

end subroutine

subroutine becke_weight(Ratom,Zatom,n_atoms,BraggSlater_radii,point,atom,weight)

! The most expensive step: get the Becke radial weights
USE kinds
implicit none

INTEGER(istd)::atom,atom1
REAL(dbl)::weight,cell,sum_weights
INTEGER(istd) :: n_atoms,Zatom(1:n_atoms)
REAL(dbl) :: Ratom(1:3,1:n_atoms),BraggSlater_radii(1:95)
REAL(dbl) :: point(1:3)

sum_weights = 0.0

do atom1=1,n_atoms,1
	call voronoi(Ratom,Zatom,n_atoms,BraggSlater_radii,point,atom1,cell)
        IF(atom1 .EQ. atom) then
                weight = cell
        END IF
	sum_weights = sum_weights + cell
END do
...
subroutine voronoi(Ratom,Zatom,n_atoms,BraggSlater_radii,point,atom_one,Vcell)

! Subroutine for the Voronoi cell for atom_one
USE kinds
implicit none

INTEGER(istd)::atom_one,atom2
REAL(dbl)::stepftion,Vcell
INTEGER(istd) :: n_atoms,Zatom(1:n_atoms)
REAL(dbl) :: Ratom(1:3,1:n_atoms),BraggSlater_radii(1:95)
REAL(dbl) :: point(1:3)
...
do WHILE((atom2 <= n_atoms) .AND. (Vcell > 10.0**(-10.0)))
        IF(atom2 .ne. atom_one) THEN

                call step_function(Ratom,Zatom,n_atoms,BraggSlater_radii,point, &
                                   atom_one,atom2,stepftion)
                Vcell = Vcell * stepftion
...
subroutine step_function(Ratom,Zatom,n_atoms,BraggSlater_radii,point,  &
                         atom_een,atom_twee,stepfunction)

! Step function between two atoms.
! This involves a product sequence of power specified above
! usually taken 3 in the Becke scheme

USE kinds; USE grid_vars

implicit none

INTEGER(istd)::atom_een,atom_twee,i
REAL(dbl)::ellipt_coord_hom,radii_ratio,coeff,dist_1,dist_2
REAL(dbl)::becke_coeff,ellipt_coord_het,stepfunction
REAL(dbl)::internucl_dist
INTEGER(istd) :: n_atoms,Zatom(1:n_atoms)
REAL(dbl) :: Ratom(1:3,1:n_atoms),BraggSlater_radii(1:95)
REAL(dbl) :: point(1:3)

dist_1 = SQRT(DOT_PRODUCT(point-Ratom(:,atom_een),point-Ratom(:,atom_een)))
dist_2 = SQRT(DOT_PRODUCT(point-Ratom(:,atom_twee),point-Ratom(:,atom_twee)))
internucl_dist = SQRT(DOT_PRODUCT(Ratom(:,atom_een)-Ratom(:,atom_twee),    &
                                  Ratom(:,atom_een)-Ratom(:,atom_twee)))

ellipt_coord_hom = (dist_1 - dist_2) / internucl_dist


IF((Zatom(atom_een).EQ.1.0) .AND. (Zatom(atom_twee).EQ.1.0)) then
        radii_ratio = 1.0
Else IF(Zatom(atom_een).EQ.1.0) then
        radii_ratio = 0.7/BraggSlater_radii(Zatom(atom_twee))
ELSE IF(Zatom(atom_twee).EQ.1.0) then
        radii_ratio = BraggSlater_radii(Zatom(atom_een))/0.7
Else
	radii_ratio = BraggSlater_radii(Zatom(atom_een))/BraggSlater_radii(Zatom(atom_twee))
END if

coeff = (radii_ratio-1.0)/(radii_ratio+1.0)
becke_coeff = coeff/((coeff*coeff)-1.0)

IF(becke_coeff > 0.5) then
        becke_coeff = 0.5
END if
IF(becke_coeff < -0.5) then
        becke_coeff = -0.5
...
ellipt_coord_het = ellipt_coord_hom + ((1.0-(ellipt_coord_hom**2.0))*becke_coeff)

stepfunction = ellipt_coord_het

do i=1,stepf_iter,1
	stepfunction = ((3.0/2.0)* stepfunction) - ((1.0/2.0)*(stepfunction**3.0))
END do

stepfunction = (1.0/2.0)*(1.0-stepfunction)

end subroutine

subroutine BraggSlater(BrSl_radii)

USE kinds

implicit none

REAL(dbl) :: BrSl_radii(1:95)

      BrSl_radii(1)=0.4724315310
      BrSl_radii(2)=0.5858150990
      BrSl_radii(3)=2.7401028810
      BrSl_radii(4)=1.9842124310
      BrSl_radii(5)=1.6062672060
      BrSl_radii(6)=1.3228082870
      BrSl_radii(7)=1.2283219810
      BrSl_radii(8)=1.1338356750
      BrSl_radii(9)=0.9448630620
      BrSl_radii(10)=0.7180959270
      BrSl_radii(11)=3.4015070250
      BrSl_radii(12)=2.8345891870
      BrSl_radii(13)=2.3621576560
      BrSl_radii(14)=2.0786987370
      BrSl_radii(15)=1.8897261250
      BrSl_radii(16)=1.8897261250
      BrSl_radii(17)=1.8897261250
      BrSl_radii(18)=1.3417055490
      BrSl_radii(19)=4.1573974750
      BrSl_radii(20)=3.4015070250
      BrSl_radii(21)=3.0235618000
      BrSl_radii(22)=2.6456165750
      BrSl_radii(23)=2.5511302690
      BrSl_radii(24)=2.6456165750
      BrSl_radii(25)=2.6456165750
      BrSl_radii(26)=2.6456165750
      BrSl_radii(27)=2.5511302690
      BrSl_radii(28)=2.5511302690
      BrSl_radii(29)=2.5511302690
      BrSl_radii(30)=2.5511302690
      BrSl_radii(31)=2.4566439620
      BrSl_radii(32)=2.3621576560
      BrSl_radii(33)=2.1731850440
      BrSl_radii(34)=2.1731850440
      BrSl_radii(35)=2.1731850440
      BrSl_radii(36)=1.6629589900
      BrSl_radii(37)=4.4408563940
      BrSl_radii(38)=3.7794522500
      BrSl_radii(39)=3.4015070250
      BrSl_radii(40)=2.9290754940
      BrSl_radii(41)=2.7401028810
      BrSl_radii(42)=2.7401028810
      BrSl_radii(43)=2.5511302690
      BrSl_radii(44)=2.4566439620
      BrSl_radii(45)=2.5511302690
      BrSl_radii(46)=2.6456165750
      BrSl_radii(47)=3.0235618000
      BrSl_radii(48)=2.9290754940
      BrSl_radii(49)=2.9290754940
      BrSl_radii(50)=2.7401028810
      BrSl_radii(51)=2.7401028810
      BrSl_radii(52)=2.6456165750
      BrSl_radii(53)=2.6456165750
      BrSl_radii(54)=2.0409042150
      BrSl_radii(55)=4.9132879250
      BrSl_radii(56)=4.0629111690
      BrSl_radii(57)=3.6849659440
      BrSl_radii(58)=3.4959933310
      BrSl_radii(59)=3.4959933310
      BrSl_radii(60)=3.4959933310
      BrSl_radii(61)=3.4959933310
      BrSl_radii(62)=3.4959933310
      BrSl_radii(63)=3.4959933310
      BrSl_radii(64)=3.4015070250
      BrSl_radii(65)=3.3070207190
      BrSl_radii(66)=3.3070207190
      BrSl_radii(67)=3.3070207190
      BrSl_radii(68)=3.3070207190
      BrSl_radii(69)=3.3070207190
      BrSl_radii(70)=3.3070207190
      BrSl_radii(71)=3.3070207190
      BrSl_radii(72)=2.9290754940
      BrSl_radii(73)=2.7401028810
      BrSl_radii(74)=2.5511302690
      BrSl_radii(75)=2.5511302690
      BrSl_radii(76)=2.4566439620
      BrSl_radii(77)=2.5511302690
      BrSl_radii(78)=2.5511302690
      BrSl_radii(79)=2.5511302690
      BrSl_radii(80)=2.8345891870
      BrSl_radii(81)=3.5904796370
      BrSl_radii(82)=3.4015070250
      BrSl_radii(83)=3.0235618000
      BrSl_radii(84)=3.5904796370
      BrSl_radii(86)=2.2676713500
      BrSl_radii(88)=4.0629111690
      BrSl_radii(89)=3.6849659440
      BrSl_radii(90)=3.4015070250
      BrSl_radii(91)=3.4015070250
      BrSl_radii(92)=3.3070207190
      BrSl_radii(93)=3.3070207190
      BrSl_radii(94)=3.3070207190
      BrSl_radii(95)=3.3070207190

! For atoms with unknown radius, guess one...
      BrSl_radii(85) = (BrSl_radii(84) + BrSl_radii(86))/2.0
      BrSl_radii(87) = (BrSl_radii(86) + BrSl_radii(88))/2.0

! Hydrogen uses double the BraggSlater radius according to Becke
      BrSl_radii(1) = BrSl_radii(1)*2.0

end subroutine

subroutine lebedev(x_leb,y_leb,z_leb,w_leb,n_leb)

    USE kinds
	implicit none

 	INTEGER::N,n_leb
    REAL(dbl) :: x_leb(1:n_leb),y_leb(1:n_leb),z_leb(1:n_leb),w_leb(1:n_leb)
    
! you may want to add more grid possibilities...
          IF (n_leb.eq.6) then
          call ld0006(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.14) then
	  call ld0014(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.26) then
	  call ld0026(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.38) then
	  call ld0038(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.50) then
	  call ld0050(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.74) then
	  call ld0074(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.86) then
	  call ld0086(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.110) then
	  call ld0110(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.146) then
	  call ld0146(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.170) then
	  call ld0170(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.194) then
	  call ld0194(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.230) then
	  call ld0230(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.266) then
	  call ld0266(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.302) then
	  call ld0302(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.350) then
	  call ld0350(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.434) then
	  call ld0434(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.590) then
	  call ld0590(x_leb,y_leb,z_leb,w_leb,N)
	else if (n_leb.eq.770) then
	  call ld0770(x_leb,y_leb,z_leb,w_leb,N)
	else if (n_leb.eq.974) then
	  call ld0974(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.1202) then
	  call ld1202(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.1454) then
	  call ld1454(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.1730) then
	  call ld1730(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.2030) then
	  call ld2030(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.2354) then
	  call ld2354(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.2702) then
	  call ld2702(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.3074) then
	  call ld3074(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.3470) then
	  call ld3470(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.3890) then
	  call ld3890(x_leb,y_leb,z_leb,w_leb,N)
	else if (n_leb.eq.4334) then
	  call ld4334(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.4802) then
	  call ld4802(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.5294) then
	  call ld5294(x_leb,y_leb,z_leb,w_leb,N)
        else if (n_leb.eq.5810) then
	  call ld5810(x_leb,y_leb,z_leb,w_leb,N)
        else
          WRITE(*,*) 'You requested a Lebedev grid that is not available'
          WRITE(*,*) 'Crashing...'
          STOP
        END if
