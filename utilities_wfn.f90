...
!   Gau_prim(prim(1:n_prim),point(1:3)) -- evaluates all the Gaussian primitives!
!                                          at a point.                          !
!   dGau_prim(prim,dprim,iprim,point) -- evaluates the iprim-th Gaussian        !
!                                        primitive and its gradient at a point. !
!   d2Gau_prim(prim,dprim,d2prim,iprim,point) -- evaluates the iprim-th Gaussian!
!                                   primitive, its gradient, and its 2nd        !
!                                   derivative tensor at a point.               !
!   d3Gau_prim(prim,dprim,d2prim,d3prim,iprim,point) -- evaluates a Gaussian    !
!                           primitive and its 1st, 2nd, and 3rd derivative      !
!                           tensors at a point.                                 !
!   primitive_type_assignments(type_exponents) -- provides the Cartesian        !
!                           exponential factors for primitive Gaussians in terms!
!                           of the *.wfn "primitive types."                     !
!   FindDet(determinant,matrix,n) -- finds the determinant of an n x n matrix.  !
!-------------------------------------------------------------------------------!
...
REAL(dbl)     :: Rvec(1:3,1:n_prim),sqdist(1:n_prim)
...
subroutine dGau_prim(prim,dprim,i_prim,point)

USE kinds; USE variables_wfn

IMPLICIT NONE

REAL(dbl) :: dprim(1:3),exp_piece,dxyz(1:3),alpha,prim,sqdist
...
!To avoid divide-by-zero errors, make sure the elements of Rvec are not too small.
DO d=1,3
   IF (ABS(Rvec(d)) < EPSILON(Rvec(d))**2) THEN
      Rvec(d) = SIGN(EPSILON(Rvec(d))**2,Rvec(d))      
   ENDIF
ENDDO
...
! d2prim(1:6) -- the second derivative of the Gaussian primitive.               !
!-------------------------------------------------------------------------------!
!                           DICTIONARY OF VARIABLES_WFN                         !
!-------------------------------------------------------------------------------!
! atm_position(1:3,1:n_atoms) -- the position of each atom.                     !
...
subroutine d2Gau_prim(prim,dprim,d2prim,i_prim,point)
...
INTEGER(istd) :: i_prim,ider,d,ijk(1:3)
REAL(dbl)     :: point(1:3),Rvec(1:3),sqdist
...
subroutine d3Gau_prim(prim,dprim,d2prim,d3prim,i_prim,point)
...
INTEGER(istd) :: i_prim,ider,d,ijk(1:3)
REAL(dbl)     :: point(1:3),Rvec(1:3),alpha,sqdist
...
end subroutine primitive_type_assignments
