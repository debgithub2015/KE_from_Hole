...
! cnt_failures -- the number of times convergence has failed.                   !
!-------------------------------------------------------------------------------!
!-------------------------------------------------------------------------------!
!-------------------------------------------------------------------------------!
...
subroutine hole_normalize(refpoints,cnt_failures)
...
INTEGER(istd) :: spin,refpoints,i,cnt_failures
...
ELSE
   ...
   !alpha-spin
   call Newton2pt(a_LDA(:,3,1),rho(:,1),cnt_failures)
   IF (multiplicity == 1) THEN
      a_LDA(:,3,2) = a_LDA(:,3,1)
   ELSE
      !Solve for beta-spin.
      call Newton2pt(a_LDA(:,3,2),rho(:,2),cnt_failures)
   ENDIF
ENDIF
...
USE kinds
USE KE_vars, ONLY: normalization_tolerance,max_iter
USE grid_vars, ONLY: n_grid,Wbecke
USE holesubroutines
...
DO   
   ...
   avg_error = SUM(abs(fnew))
   norm_error = ABS(SUM(fnew))
   ...
   IF (max_error < MIN(normalization_tolerance**3,SQRT(EPSILON(normalization_tolerance)))    &
       .and. avg_error < MIN(normalization_tolerance**3,SQRT(EPSILON(normalization_tolerance)))) THEN
...
! p -- just a geometric factor between zero and one.                            !
!                                                                               !
...
!                                                                               !
! i,j,k,l -- counters.                                                          !
! cnt_failures -- A counter that indicates how many times we have failed        !
!                       to converge.                                            !
!-------------------------------------------------------------------------------!

subroutine Newton2pt(value,rho,cnt_failures)
...
REAL(dbl) :: fguess(1:n_grid),pred_error,fratio(1:n_grid),p
...
INTEGER(istd) :: iter,i,j,k,l,iter_bad,iter_slow,cnt_failures

INTEGER :: iarray(1:1),iloc                         !For debugging.
...
IF (Jvectors < 0) THEN
   ...
   Jvectors = 5
ENDIF
IF (Jdiag_update <= 0) THEN
   !We *NEVER* compute the Jacobian.
   Jdiag_update = -2*max_iter
ENDIF
...
1117 FORMAT(20X,8(2X,A8))
1118 FORMAT(20X,8(7X,I3))
1119 FORMAT(20X,I3,3X,A,ES12.5,A,I3)
1120 FORMAT(20X,I3,3X,A,I3)
...
DO   
   ...
   !
   !When you are having a hard time converging, commenting
   !out these lines can be helpful.  The following code tries to switch on
   !backward steps when the convergence is very slow, just in case it might help.
   IF (iter > 3 .and. .not. (MOD(iter,5) == 0 .and. 3*COUNT(Lbwardstep) > n_grid)) THEN
      WHERE(Lbwardstep)
      ...
      anew = MAX(anew,0.0_dbl)
   ELSE
      !We only allow backwards steps very occassionally; they don't seem to help
      !much.
      WRITE(*,'(20X,A)') "===Allow Backwards Steps.==="
   ENDIF
   
   !Initialize array for "good backward" steps.
   Lgoodbward(:) = .false.
   ...
   !Call subroutine to evaluate the error in the normalization of the hole.
   IF ((iter == 1 .and. Jdiag_update > 0)                                       &
       .or. (MOD(iter,Jdiag_update) == 0)) THEN
      !We need to compute the Jacobian.
      WRITE(*,1111) "   Updated Jacobian's Diagonal"
      call normalization2ptJacobian(rho,anew,fnew,Jiinew)
   ELSE IF (iter == 1 .and. Jdiag_update <= 0) THEN
      !We never compute the Jacobian at all.  The initial Jacobial is the identity
      !matrix with the signs of the function values.  So
      WRITE(*,1111) '   No Jacobian Computations ever.'
      call normalization2pt(rho,anew,fnew)
      Jiinew = SIGN(1.0_dbl,fnew)      
   ELSE 
   ...
   !IF (debug) THEN
   !   !Check Jacobian
   !   iarray = MAXLOC(ABS(fnew))
   !   iloc = iarray(1)
   !   WRITE(10,*) iter,'    rho, kf ',rho(iloc),iloc,anew(iloc),'value, Jacobian ',fnew(iloc),Jiinew(iloc)
   !   FLUSH(10)
   !ENDIF
   ...
   avg_error_new = SUM(abs(fnew))   
   max_error = MAXVAL(ABS(fnew))
   norm_error = ABS(SUM(fnew))
   pred_error = SUM(abs(fguess))  
   ...
   IF (iter_bad > 0) THEN
      !The last step was rejected.  This indicates that the trust radius is too large.
      !Reset the trust radius to the step size (which is being steadily decreased
      !elsewhere in the program).
      Global_trust = step_size
   ENDIF
   IF (COUNT(Lshort) > 2*COUNT(Llong) .and. Lscalestep) THEN
      ...
      Global_Trust = Global_Trust*MIN(COUNT(Lshort)*1.0_dbl/(2*COUNT(Llong)+1),2.0_dbl)
   ELSE IF (COUNT(LLong) > 2*COUNT(Lshort)) THEN
      ...
      IF (Lscalestep) THEN
         ...
         !more agressively.
         Global_Trust = Global_Trust/MIN( COUNT(Llong)*1.0_dbl/(2*COUNT(Lshort)+1), 2.0_dbl)
      ELSE
         ...
         !of the current step.
         Global_Trust = MAX(step_size/MIN( COUNT(Llong)*1.0_dbl/(2*COUNT(Lshort)+1), 2.0_dbl), &
                            Global_Trust/4)
      ENDIF
   ELSE
      ...
      CONTINUE
   ENDIF
   WRITE(*,1111) '   new global trust radius: ',Global_trust
   ...
   IF (debug .and. iter_bad ==0) THEN
      WRITE(*,*) '          Percentage of',n_grid, ' points that: ' 
      WRITE(*,1112) 'improved','tooshort',' toolong','in-trust','de-trust','  pruned','backward','goodbwrd'
   ...
   IF (max_error < normalization_tolerance                                   &
      ...
      WRITE(*,*) ' '
      WRITE(*,*) ' '
      cnt_failures = -5  !Set equal to a negative number to indicate convergence.
      EXIT
   ...
   !Decide whether the new step is better or worse than the old step.  We don't
   !force this to be *perfect*, but we do want to avoid having "big upward moves."
   !The square of the integration error is a reasonable criterion.  (So is the
   !integration error itself, but I found that this gave too many upward moves.)
   !The final criterion I settled on was that we would not allow upward moves that
   !caused the error to increase by more than .1% of the normalization tolerance.  
   IF ((avg_error_new - avg_error_best) > normalization_tolerance/1000) THEN
   !BADSTEP BADSTEP BADSTEP BADSTEP BADSTEP BADSTEP BADSTEP BADSTEP BADSTEP BADSTEP BADSTEP BADSTEP
      !There was no improvement in this step.  Try again with a smaller step.
      anew = abest + step/2
      
      WRITE(*,1115) iter_bad, ' The error went ***UP*** by  ',avg_error_new-avg_error_best
      IF (iter_bad > 0) THEN
         WRITE(*,1116) '    maximum absolute error: ',max_error
         WRITE(*,1116) '      total absolute error: ',avg_error_new
         WRITE(*,1116) 'predicted total abs. error: ',pred_error
         WRITE(*,1116) '       normalization error: ',norm_error
         WRITE(*,1116) '                 step size: ',step_size
            WRITE(*,1116) '       global trust radius: ',Global_trust
         WRITE(*,1117) 'improved','tooshort',' toolong','in-trust','de-trust','  pruned','backward','goodbwrd'
         WRITE(*,1118) NINT(COUNT(Limprove)*100.0_dbl/n_grid),                 &
                       NINT(COUNT(Lshort)*100.0_dbl/n_grid),                   &
                       NINT(COUNT(Llong)*100.0_dbl/n_grid),                    &
                       NINT(COUNT(Lincrtrust)*100.0_dbl/n_grid),               &
                       NINT(COUNT(Ldecrtrust)*100.0_dbl/n_grid),               &
      ...
      ENDIF
      IF (iter_bad > 30) THEN 
         !This seems hopeless :-( .  We reduced the step by almost 10 orders of magnitude
         !and still no luck!!
         WRITE(*,*) ' '
   ...
   !GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP 
      ...
      IF ( ((MAXVAL(list_error) - MINVAL(list_error))/5*(max_iter-iter)        &
           < normalization_tolerance)) THEN
         !Very slow convergence.  Too slow to converge even if linear from here on
         !out.  
         WRITE(*,*) '===Very Slow Convergence==='
         iter_slow = iter_slow + 1
         IF (iter_slow > 11) THEN
            WRITE(*,*) ' '
            WRITE(*,*) '*** TOTAL FAILURE TO CONVERGE TO DESIRED ACCURACY ***.'
            WRITE(*,*) '*** :-( :-( :-( :-( :-( :-( :-( :-( :-( :-( :-(   ***.'
            WRITE(*,*) 'Will retry with more compact grid.'
            WRITE(*,*) ' ' 
            cnt_failures = cnt_failures + 1
            EXIT      
         ENDIF
      ...
      !Reset the counters for inner iterations.
      iter_bad = 0
      !Increment iteration.
      iter = iter + 1
...
!variable become negative.  (a = 0 is absurd!!)  When it wants to go negative,
!we allow it to approach 1/2 of the way to zero.  It'll get to zero eventually 
!(the value goes down 3 orders of magnitude every 10 steps), but not too fast.
WHERE (fullstep < -1*anow/2)
      step = -1*anow/2
      Lcutstep = .true.
ELSEWHERE
      step = fullstep
...
ELSE
   Lscalestep = .true.
   step = step*Global_trust/trust_compare
