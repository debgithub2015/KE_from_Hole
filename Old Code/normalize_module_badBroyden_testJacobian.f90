...
REAL(dbl) :: f0,h                                     !For debugging.
INTEGER :: iarray(1:1),iloc                         !For debugging.
REAL(dbl) :: fplus(1:n_grid),fplus2(1:n_grid),aplus(1:n_grid),aplus2(1:n_grid)
...
DO   
   ...
   IF (debug) THEN
      ...
      iloc = iarray(1)
      f0 = fnew(iloc)
      h = MAX(anew(iloc)/1000,1.0D-5)
      aplus2 = anew; aplus = anew
      aplus2(iloc) = aplus2(iloc) + 2*h
      aplus(iloc) = aplus(iloc) + h
      call normalization2pt(rho,aplus,fplus)
      call normalization2pt(rho,aplus2,fplus2)
      WRITE(10,*) ' '
      WRITE(10,*) iter,h,iloc,rho(iloc),anew(iloc),Wbecke(iloc)
      WRITE(10,*) f0,Jiinew(iloc),(fplus(iloc) - f0)/h,              &
                  (fplus2(iloc)-2*fplus(iloc)+f0)/h**2
      WRITE(10,*) Jiinew(iloc)/((fplus(iloc) - f0)/h)
      iloc = iter*1.0_dbl/max_iter*n_grid
      f0 = fnew(iloc)
      h = MAX(anew(iloc)/1000,1.0D-5)
      aplus2 = anew; aplus = anew
      aplus2(iloc) = aplus2(iloc) + 2*h
   ...
   IF (COUNT(Lshort) > 2*COUNT(Llong) .and. Lscalestep) THEN
      ...
      Global_Trust = Global_Trust*MIN(COUNT(Lshort)/(2*COUNT(Llong)+1),2)
   ELSE IF (COUNT(LLong) > 1.5*COUNT(Lshort)) THEN
      ...
      IF (Lscalestep) THEN
         ...
         !more agressively.
         Global_Trust = Global_Trust/MIN( COUNT(Llong)/(1.5*COUNT(Lshort)+1), 2.0_dbl)
      ELSE
   ...
   IF ((avg_error_new - avg_error_best) > integration_error/10) THEN
   ...
   !GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP 
      ...
      IF ( ((MAXVAL(list_error) - MINVAL(list_error))/5*(max_iter)        &
           < normalization_tolerance/10)) THEN
         !Very slow convergence.  Too slow to converge even if linear from here on
         !out.  
         WRITE(*,*) '===Very Slow Convergence==='
         !We will try to slowly beat the system into submission.  We find the largest
         !errors, and then try to take a step that beats those bad elements into submission.
         !We use the Lcutstep array to hold the worst cases.
         iter_slow = iter_slow + 1
         IF (iter_slow > 10) THEN
            WRITE(*,*) ' '
            WRITE(*,*) '*** TOTAL FAILURE TO CONVERGE TO DESIRED ACCURACY ***.'
            WRITE(*,*) '*** :-( :-( :-( :-( :-( :-( :-( :-( :-( :-( :-(   ***.'
            WRITE(*,*) ' '  
