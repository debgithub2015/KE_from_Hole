...
IF (refpoints == 0) THEN
   ...
   ENDFORALL
ELSE
   !Initialize to the previous value in the array
   a_LDA(:,refpoints,:) = a_LDA(:,refpoints-1,:)
   
   !Call Newton's method to do the normalization,
   !alpha-spin
   call Newton(a_LDA(:,refpoints,1),rho(:,1),normalization_tolerance,refpoints)
   IF (multiplicity == 1) THEN
      a_LDA(:,refpoints,2) = a_LDA(:,refpoints,1)
   ELSE
      !Solve for beta-spin.
      call Newton(a_LDA(:,refpoints,2),rho(:,2),normalization_tolerance,refpoints)
   ENDIF
ENDIF
...
ELSE
   !In this version of the program we always compute all points
   compute_flag(:) = .true.
