...
DO   
   ...
   !GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP GOODSTEP 
      ...
      IF ( ((MAXVAL(list_error) - MINVAL(list_error))/5*(max_iter-iter+1)        &
           < normalization_tolerance*.1) .and. (iter >= 30)               &
                   .and. (iter_slow < 61) .and. (MOD(iter,10) == 0)) THEN
         !Very slow convergence.  Too slow to converge even if linear from here on
         !out.  
         WRITE(*,*) '===Very Slow Convergence===...will try to hammer down the error little-by-little.'
         ...
         max_error = MAXVAL(ABS(fbest))
         IF (iter_slow > 1) THEN
            WHERE(Limprove .and. Lcutstep)
                 Limprove = .true.
            ELSEWHERE
                 Limprove = .false.
            ENDWHERE
            WRITE(*,1120) iter_slow, ' % of changed points that improved in previous step ',  &
                          NINT(COUNT(Limprove)*100.0_dbl/COUNT(Lcutstep))
         ENDIF
         !Choose every, say, 10th point.
         Lcutstep = .false.
         DO i=MOD(iter_slow,20),n_grid,20
            Lcutstep(i) = .true.
         ENDDO
         !Take a brute-strength step to try to drive down the error.
         WHERE (Lcutstep)
               anew = -1*MAX(fbest,abest/50)
               Lcutstep = .true.
         ELSEWHERE
               anew = abest
               Lcutstep = .false.
         ENDWHERE
         WRITE(*,1119) iter_slow, ' step size is ', SUM(ABS(anew-abest)),                          &
                            ' updating x% of the points ',NINT(COUNT(Lcutstep)*100.0_dbl/n_grid)
