!-------------------------------------------------------------------------------!
!                           INOUT_MODULE                                        !
!                                                                               !
! Contains subroutines for input and output.                                    !
!-------------------------------------------------------------------------------!

MODULE INOUT_MODULE

CONTAINS

!-------------------------------------------------------------------------------!
! read_input -- driver routine for input.                                       !
! input_file -- reads in the parameters that define the grid and the system.    !
! getgrid -- constructs the grid.                                               !
! get_density -- constructs the atomic density and its gradient on the grid.    !
! check_normalization -- checks the normalization of the density.               !
! write_output -- summarizes the output.                                        !
!-------------------------------------------------------------------------------!
!*******************************************************************************!
!                              read_input                                       !
!                                                                               !
! A driver subroutine that reads the input file, constructs grids for           !
! integration and differentiation, etc..                                        !
!-------------------------------------------------------------------------------!

subroutine read_input

IMPLICIT NONE

call Input_file()   !Reads the input file containing the grid-parameters
                    !and the atomic number.

end subroutine read_input

!-------------------------------------------------------------------------------!
!                               Input_file                                      !
!                                                                               !
! This subroutine reads in the input file.  It is adapted from Rogelio.         !
!-------------------------------------------------------------------------------!
!                          INTERNAL DICTIONARY                                  !
!                                                                               !
! comment -- a character value to identify comments in the file.                !
!-------------------------------------------------------------------------------!
!                       VARIABLES FROM VARS_GRID                                !
! ----The definitive statement of these definitions is found in the        ---- !
! ----vars_grid module!!!                                                  ---- !
! Nr -- minimum number of radial points.                                        !
! Rm -- Rm parameter for constructing the radial grid.  Similar to Becke.       !
! Lebedev_order -- Number of Lebedev points.                                    !
! Z_Atomic -- atomic number                                                     !
! exp_step -- the step size, h, for evaluating derivatives is EPSILON**exp_step !
! h -- step size for evaluating derivatives                                     !
!-------------------------------------------------------------------------------!

SUBROUTINE Input_file

USE vars_grid; USE vars_density
USE kinds

IMPLICIT NONE
CHARACTER(len=1) :: comment	! (internal) Just a character variable to identify comments in the file


! Open input file.
OPEN(UNIT=5, STATUS='OLD',FILE='InputFile.inp',ACTION='READ')

DO
   READ(5,*) comment
   IF (comment /= '!')THEN !This line contains data, not a comment!
      backspace(5)
      READ(5,*) Nr		   ! The number of radial points is read
      EXIT
   ENDIF
ENDDO

DO
   READ(5,*) comment
   IF (comment /= '!')THEN !This line contains data, not a comment!
      backspace(5)
      READ(5,*) Rm			! The parameter for creating the radial grid is read			
							! Recommended values for each of the corresponding atoms are
							! included in the input file itself
      EXIT
   ENDIF
ENDDO


DO
   READ(5,*) comment
   IF (comment /= '!')THEN !This line contains data, not a comment!
      backspace(5)
      READ(5,*) Lebedev_order  ! Number of angular grids in the Lebedev grid
							   ! The code permits to choose Lebedev grids with 38, 50, 86, 110,
							   ! 170, 350 and 1202 angular points
      EXIT
   ENDIF
ENDDO



DO
   READ(5,*) comment
   IF (comment /= '!')THEN !This line contains data, not a comment!
      backspace(5)
      READ(5,*) Z_Atomic		! The atomic number is read
      EXIT
   ENDIF
ENDDO

DO
   READ(5,*) comment
   IF (comment /= '!')THEN !This line contains data, not a comment!
      backspace(5)
      READ(5,*) exp_step		! exponent controlling the step size for the
      EXIT                      !finite difference derivatives is read
   ENDIF
ENDDO

h = EPSILON(h)**exp_step

! The file is closed.
CLOSE(5)

END SUBROUTINE Input_File

!-------------------------------------------------------------------------------!
!                               GetGrid                                         !
!                                                                               !
! This subroutine constructs the grid.  It is adapted from Rogelio.  One *big*  !
! difference is that now the grid is in *CARTESIAN* coordinates, because that   !
! is a *LOT* easier for this application.                                       !
!                                                                               !
! This subroutine reads UNIT=15 'Lebedev_RECgrid.txt.' This file contains the   !
! angular points in cartesian coordinates (x,y,z).  The tensor product of the   !
! radial and angular grids is performed.  Then points are added for computing   !
! the finite-difference derivatives.                                            !
!-------------------------------------------------------------------------------!
!                          INTERNAL DICTIONARY                                  !
!                                                                               !
! W_ang(:) -- contains the angular weights.                                     !
! X_ang(:,:) -- contains the angular points                                     !
! W_R(:) -- contains the radial weights.                                        !
! R(:,:) -- contains the radial points.                                         !
! N_ang -- number of angular grid points.                                       !
! N_R -- number of radial grid points.                                          !
! i,j,k -- counters.                                                            !
! istat -- used to diagnose the end of the file.                                !
! Rmax -- used to fix the endpoint of the radial grid.                          !
! Lebedev_RECgrid -- filename of the Lebedev grid.                              !
! label -- a label.  Just a temporary variable.                                 !
! ipt -- counts the grid points.                                                !
!-------------------------------------------------------------------------------!
!                       VARIABLES FROM VARS_GRID                                !
! ----The definitive statement of these definitions is found in the        ---- !
! ----vars_grid module!!!                                                  ---- !
! Nr -- minimum number of radial points.                                        !
! Rm -- Rm parameter for constructing the radial grid.  Similar to Becke.       !
! Lebedev_order -- Number of Lebedev points.                                    !
! Z_Atomic -- atomic number                                                     !
! exp_step -- the step size, h, for evaluating derivatives is EPSILON**exp_step !
! h -- step size for evaluating derivatives                                     !
! ngrid -- number of points on the grid, including the points used to compute   !
!          the derivatives.                                                     !
! X(1:3,1:ngrid) -- the grid points.                                            !
! Wint(1:ngrid) -- the integration weights.                                     !
! h -- the step size for evaluating the derivatives, EPSILON**exp_step.         !
!-------------------------------------------------------------------------------!

SUBROUTINE getgrid 

IMPLICIT NONE

USE kinds
USE vars_grid

INTEGER :: N_ang, N_R, i,j,k,istat,ipt

REAL(dbl),ALLOCATABLE :: W_ang(:),W_R(:),X_ang(:,:),R(:)
REAL(dbl)             :: Rmax,label						
CHARACTER(len=20) :: Lebedev_RECgrid	

! Choose the right file to read in the Lebedev points.
IF (Lebedev_order == 38) THEN
	Lebedev_RECgrid = 'Lebedev_38_REC.txt'
ELSE IF (Lebedev_order == 50) THEN
	Lebedev_RECgrid = 'Lebedev_50_REC.txt'
ELSE IF (Lebedev_order == 86) THEN
	Lebedev_RECgrid = 'Lebedev_86_REC.txt'
ELSE IF (Lebedev_order == 110) THEN
	Lebedev_RECgrid = 'Lebedev_110_REC.txt'
ELSE IF (Lebedev_order == 170) THEN
	Lebedev_RECgrid = 'Lebedev_170_REC.txt'
ELSE IF (Lebedev_order == 350) THEN
	Lebedev_RECgrid = 'Lebedev_350_REC.txt'
ELSE IF (Lebedev_order == 1202) THEN
	Lebedev_RECgrid = 'Lebedev_1202_REC.txt'
END IF

! Open the file where the Lebedev grid points (RECTANGULAR) are stored.
OPEN(UNIT=15,STATUS='old',FILE=Lebedev_RECgrid,ACTION='read',IOSTAT=ierror)

! Count the number of points in the grid and allocate the arrays.
N_ang = 0
DO 
   READ(15,*,IOSTAT=ierror)
   IF(ierror /= 0) EXIT     !If the end of file has been reached, then it exists the file
   N_ang = N_ang + 1	    !If a data is contained in that line, then the counter is increased.
END DO

ALLOCATE(W_ang(1:N_ang),X_ang(1:3,1:N_ang)
ALLOCATE(W_R(1:Nr),R(1:NR))
N_R = NR

!Rewind the file to the beginning and read the points.
REWIND(15)
DO i = 1, N_ang
   !The data contained in the file are:
   !the label of the point, the weight, the x coordianted, y coordinate and z coodinate
   !respectively
   READ(15,*) label, W_ang(i), X_ang(1,i), X_ang(2,i), X_ang(3,i)
ENDDO
CLOSE(15)

!Rmax is a parameter related to the radial grid.  I have no idea what it means,
!but I trust Rogelio. . . .  He says "it is set to avoid very small values of the
!density.  It seems to be more-or-less the most distant radius in the grid.


! This parameter is defined based on Rm. Rogelio says that it is set to avoid 
! very small values of the density.  The radial grid runs from
! R = Rmax/(Nr+1)**2 to R = Rmax*(Nr**2/4).  
! So this ensures that Rm is the maximum distance in the grid. 
! n.b. I've rearrangd this because exponentiating to integer powers is MUCH faster,
!      and exponentiating integers is faster than exponentiating real numbers.  The
!      only concern I have is that one might "overflow" the integer range. So I did
!      use a *couple* real numbers.  I'm assuming that Nr**2 is small enough, however.
!
Rmax = 4*Rm/Nr**2

! The radial grid is generated
DO i = 1, Nr
   R(i) = i**2*Rmax/(Nr + 2 - i)**2
   W_R(i) = 2*(Nr + 2)*i_dbl**5*Rmax**3/((Nr + 2 - i)_dbl)**7
END DO

!We are doing a tensor product grid.  For eact integration grid point we add six
!extra points for doing derivatives.
ngrid = 7*N_R*N_ang
ALLOCATE(W(1:ngrid),R(1:3,1:ngrid))

ipt = 1

DO i = 1,N_R      !Loop over radial points
   DO j = 1, Nang !Loop over angular points
      !The first point of each "set of seven" is the integration point
      Wint(ipt) = W_R(i)*W_ang(j)
      X(1:3,ipt) = R(i)*X_ang(1:3,j)
      !The next six points have integration weight ZERO
      Wint(ipt+1:ipt+6) = 0.0_dbl
      !They are offset +/- h in each direction.  Start by assuming they are the
      !same as the "central point" in the finite-difference expression:
      X(1:3,ipt+1:ipt+6) = X(1:3,ipt)
      !then nudge them away
      X(1,ipt+1) = X(1,ipt+1) - h
      X(1,ipt+2) = X(1,ipt+2) + h
      X(2,ipt+3) = X(2,ipt+3) - h
      X(2,ipt+4) = X(2,ipt+4) + h
      X(3,ipt+5) = X(3,ipt+5) - h
      X(3,ipt+6) = X(3,ipt+6) + h
      ipt = ipt + 7
   ENDDO
ENDDO
      
DEALLOCATE(R,W_r,W_ang,X_ang)

END SUBROUTINE getgrid

!-------------------------------------------------------------------------------!
!                               Get_density                                     !
!                                                                               !
! This subroutine constructs the density and the gradient of the density.       !
! It is adapted from Rogelio.                                                   !
!                                                                               !
! The density is:                                                               !
!   rho(:) = SUM coeff(i)*EXP(-expon(i)*R(I)**2                                 !
!                                                                               !
! If -101 < Z_atomic < 0 then this constructs the density of the NONINTERACTING !
!     atom with charge |Z_atomic|.                                              !
! If Z_atomic < -100 then this constructs the density of |Z_atomic| - 100       !
!     NONINTERACTING harmonically bound electrons.                              !
!-------------------------------------------------------------------------------!
!                          INTERNAL DICTIONARY                                  !
!                                                                               !
! coeff(1:20) -- coefficients of the Gaussians.                                 !
! expon(1:20) -- exponents of the Gaussians.                                    !
! Rsq -- squared distance of the point from the origin.                         !
!-------------------------------------------------------------------------------!
!                       VARIABLES FROM VARS_GRID                                !
! ----The definitive statement of these definitions is found in the        ---- !
! ----vars_grid module!!!                                                  ---- !
! Z_Atomic -- atomic number                                                     !
! h -- step size for evaluating derivatives                                     !
! ngrid -- number of points on the grid, including the points used to compute   !
!          the derivatives.                                                     !
! X(1:3,1:ngrid) -- the grid points.                                            !
! Wint(1:ngrid) -- the integration weights.                                     !
!-------------------------------------------------------------------------------!
!                       VARIABLES FROM VARS_density                             !
! ----The definitive statement of these definitions is found in the        ---- !
! ----vars_density module!!!                                               ---- !
! rho(:) -- the density at a point                                              !
! Drho(1:3,:) -- the gradient of the density at a grid point.                   !
!-------------------------------------------------------------------------------!

subroutine get_density

IMPLICIT NONE

USE kinds; USE vars_density

REAL(dbl) :: coeff(1:20),expon(1:20),Rsq
INTEGER :: i   !a counter

!Allocate arrays for the density
ALLOCATE(rho(1:ngrid),Drho(1:3,1:ngrid))

!Initialize everything:
coeff = 0_dbl; expon = 0_dbl; rho = 0_dbl; Drho = 0_dbl

IF (Z_Atomic > 0) THEN
   !Get the numbers that define the density
   call get_density_constants(coeff,expon)
   !Compute the density and the gradient for each point
   DO i=1,ngrid
      Rsq = SUM(X(1:3)**2)
      rho(i)    = DOT_PRODUCT(coeff(1:20), EXP(-1_dbl*EXPON(1:20)*Rsq))
      Drho(1,i) = DOT_PRODUCT(-2*X(1,i)*coeff(:)*EXPON(:), EXP(-1_dbl*EXPON(:)*Rsq))
      Drho(2,i) = DOT_PRODUCT(-2*X(2,i)*coeff(:)*EXPON(:), EXP(-1_dbl*EXPON(:)*Rsq))
      Drho(3,i) = DOT_PRODUCT(-2*X(3,i)*coeff(:)*EXPON(:), EXP(-1_dbl*EXPON(:)*Rsq))
   ENDDO

   !Check the normalization of the density
   call check_normalization() 

   !Divide density by two to get spin density
   rho = rho/2
   Drho = Drho/2
ELSE
   WRITE(101,*) 'This feature not implemented yet.  Happy programming!'
   WRITE(*,*) 'This feature not implemented yet.  Happy programming!'
ENDIF


end subroutine get_density

!-------------------------------------------------------------------------------!
!                       check_normalization                                     !
!                                                                               !
! This subroutine checks the grid by confirming the normalization of the density!
!-------------------------------------------------------------------------------!
!                               DICTIONARY OF LOCAL VARIABLES                   !
! normalization -- predicted normalization of the density.                      !
!-------------------------------------------------------------------------------!
!-------------------------------------------------------------------------------!
!                       VARIABLES FROM VARS_GRID                                !
! ----The definitive statement of these definitions is found in the        ---- !
! ----vars_grid module!!!                                                  ---- !
! Z_Atomic -- atomic number                                                     !
! ngrid -- number of points on the grid, including the points used to compute   !
!          the derivatives.                                                     !
! X(1:3,1:ngrid) -- the grid points.                                            !
! Wint(1:ngrid) -- the integration weights.                                     !
! N_ang -- number of angular grid points.                                       !
! N_R -- number of radial grid points.                                          !
!-------------------------------------------------------------------------------!
!                       VARIABLES FROM VARS_density                             !
! ----The definitive statement of these definitions is found in the        ---- !
! ----vars_density module!!!                                               ---- !
! rho(:) -- the density at a point                                              !
!-------------------------------------------------------------------------------!

subroutine check_normalization

IMPLICIT NONE

USE kinds; USE vars_grid; USE vars_density

REAL(dbl) :: normalization

normalization = 0.0_dbl

!Compute the integral.  If problems in this statement use BLAS and DDOT.
normalization = DOT_PRODUCT(Wint,rho)

!Print output to screen.
WRITE(*,*) 'The correct number of electrons is:    ',Z_Atomic
WRITE(*,*) 'The computed number of electrons is:   ',normalization
WRITE(*,*) 'The absolute error is equal to:        ',ABS(Z_Atomic-normalization)
WRITE(*,*) 'The number of radial grid points is:   ',N_R
WRITE(*,*) 'The number of angular grid points is:  ',N_ang
WRITE(*,*) 'The total number of 3D grid points is: ',ngrid

!Print output to file
WRITE(101,*) 'The correct number of electrons is:    ',Z_Atomic
WRITE(101,*) 'The computed number of electrons is:   ',normalization
WRITE(101,*) 'The absolute error is equal to:        ',ABS(Z_Atomic-normalization)
WRITE(101,*) 'The number of radial grid points is:   ',N_R
WRITE(101,*) 'The number of angular grid points is:  ',N_ang
WRITE(101,*) 'The total number of 3D grid points is: ',ngrid

end subroutine check_normalization


!-------------------------------------------------------------------------------!
!                               Get_density_constants                           !
!                                                                               !
! This subroutine gets the constants that define the density.                   !
!-------------------------------------------------------------------------------!

subroutine get_density_constants(coeffs,expon)

USE kinds
USE vars_grid, ONLY: Z_Atomic

IMPLICIT NONE

REAL(dbl) :: coeffs(1:20)  !The coefficients.
REAL(dbl) :: expon(1:20)   !The exponents.

! The following IF condition is used to choose the coefficients and exponents
! according to the atomic number.
! A complete set of coefficients and exponents for a number of atoms can be found
! in the file "rho_G_Ayers.txt"

IF (Z_Atomic == 1) THEN
    Coeffs(1) = 1.6974473253E-02	 
    Expon(1) = 1.1312807919E+02
    Coeffs(2) = 5.3025697740E-02	 
    Expon(2) = 6.6470125536E+00
    Coeffs(3) = 3.0592022212E-02	 
    Expon(3) = 5.3772337038E-01
    Coeffs(4) = 5.8707537674E-02	 
    Expon(4) = 1.2094177622E+00
    Coeffs(5) = 3.9342138430E-03	 
    Expon(5) = 3.5105124063E+03
    Coeffs(6) = 6.1214629017E-03	 
    Expon(6) = 9.5506080498E+02
    Coeffs(7) = 3.8470423701E-02	 
    Expon(7) = 1.6399410871E+01
    Coeffs(8) = 4.7015082653E-03	 
    Expon(8) = 2.3950580176E-01
    Coeffs(9) = 4.2305722453E-03	 
    Expon(9) = 4.8487238729E+08
    Coeffs(10) = 6.3961685151E-02	 
    Expon(10) = 2.7908454796E+00
    Coeffs(11) = 1.0719169010E-02	 
    Expon(11) = 3.2100441066E+02
    Coeffs(12) = 2.6066777824E-02	 
    Expon(12) = 4.2095237219E+01
    Coeffs(13) = 8.0914384550E-04	 
    Expon(13) = 1.4706017501E+03
ELSE IF (Z_Atomic == 2) THEN
    Coeffs(1) = 3.72587013930776000E-01	
    Expon(1) = 3.22049584257601000E+01
    Coeffs(2) = 3.03125226640509000E-01	
    Expon(2) = 5.92862791831091000E+01
    Coeffs(3) = 3.55156193642548000E-01	
    Expon(3) = 3.23706355904431000E+00
    Coeffs(4) = 6.81872637173492000E-02	
    Expon(4) = 1.61835956608224000E+03
    Coeffs(5) = 2.62709918168175000E-02	
    Expon(5) = 1.35094660425194000E+04
    Coeffs(6) = 9.62863544507734000E-02	
    Expon(6) = 1.03045514096093000E+00
    Coeffs(7) = 3.77920862379295000E-02	
    Expon(7) = 8.61567791157559000E+04
    Coeffs(8) = 2.22337095698631000E-01	
    Expon(8) = 1.83287991704299000E+00
    Coeffs(9) = 1.30520673406063000E-01	
    Expon(9) = 4.23862904237793000E+02
    Coeffs(10) = 2.24296355532833000E-02	
    Expon(10) = 5.73160579903299000E-01
    Coeffs(11) = 9.37424202617610000E-02	
    Expon(11) = 8.26383780782377000E+02
    Coeffs(12) = 2.39218920951108000E-01	
    Expon(12) = 1.11827115869475000E+02
    Coeffs(13) = 1.52543877042631000E-03	
    Expon(13) = 3.08911634633236000E-01
    Coeffs(14) = 4.33436114721961000E-01	
    Expon(14) = 1.78294322243255000E+01
    Coeffs(15) = 1.95686993455118000E-02	
    Expon(15) = 2.96150976908799000E+04
    Coeffs(16) = 4.42597495414955000E-01	
    Expon(16) = 5.69232235232830000E+00
    Coeffs(17) = 3.60435849926551000E-02	
    Expon(17) = 6.49084148432205000E+03
    Coeffs(18) = 1.80913727074189000E-01	
    Expon(18) = 2.16682704507575000E+02
    Coeffs(19) = 4.95652456223779000E-02	
    Expon(19) = 3.21124214858286000E+03
    Coeffs(20) = 4.64613861668947000E-01	
    Expon(20) = 1.00265876351762000E+01
ELSE IF (Z_Atomic == 3) THEN
    Coeffs(1) = 1.73910306538455000E+00	
    Expon(1) = 1.43720065982558000E+01
    Coeffs(2) = 4.68131319934221000E-01	
    Expon(2) = 9.47115407405902000E+02
    Coeffs(3) = 9.32335316863302000E-01	
    Expon(3) = 4.68404489605021000E+00
    Coeffs(4) = 1.06095520157587000E+00	
    Expon(4) = 1.50180919767390000E+02
    Coeffs(5) = 7.48828729260398000E-02	
    Expon(5) = 6.97720209010184000E+04
    Coeffs(6) = 1.34593678495505000E+00	
    Expon(6) = 8.22406324879563000E+01
    Coeffs(7) = 2.59141396717078000E-01	
    Expon(7) = 3.48212688033284000E+03
    Coeffs(8) = 1.44482604658001000E-01	
    Expon(8) = 1.41870295819931000E+04
    Coeffs(9) = 5.56018291363616000E-03	
    Expon(9) = 8.29226753237294000E-01
    Coeffs(10) = 1.93319917802891000E-01	
    Expon(10) = 6.87781643828071000E+03
    Coeffs(11) = 1.04020652065583000E-01	
    Expon(11) = 3.07166124044656000E+04
    Coeffs(12) = 1.77314403758160000E+00	
    Expon(12) = 2.53806527752997000E+01
    Coeffs(13) = 4.08640248217337000E-01	
    Expon(13) = 2.67386831009337000E+00
    Coeffs(14) = 8.11978015398148000E-01	
    Expon(14) = 2.75327500805785000E+02
    Coeffs(15) = 9.18810498055641000E-02	
    Expon(15) = 1.51575486153528000E+00
    Coeffs(16) = 3.50628448503757000E-01	
    Expon(16) = 1.80098086731805000E+03
    Coeffs(17) = 1.38245998390784000E-01	
    Expon(17) = 2.15905017006394000E+05
    Coeffs(18) = 1.44243672851088000E+00	
    Expon(18) = 8.19601785466215000E+00
    Coeffs(19) = 6.16264530266749000E-01	
    Expon(19) = 5.07267465132135000E+02
    Coeffs(20) = 1.61224197560743000E+00	
    Expon(20) = 4.53855482854463000E+01
ELSE IF (Z_Atomic == 4) THEN
    Coeffs(1) = 2.16196602500000000E-02	
    Expon(1) = 1.71317162000000000E-01
    Coeffs(2) = 1.68555951400000000E-02	
    Expon(2) = 3.77138325500000000E-01
    Coeffs(3) = 7.42880343800000000E-01	
    Expon(3) = 4.02346344500000000E+00
    Coeffs(4) = 4.00198500800000000E+00	
    Expon(4) = 8.85726945700000000E+00
    Coeffs(5) = 5.57284805300000000E+00	
    Expon(5) = 1.94984304700000000E+01
    Coeffs(6) = 7.02282986600000000E+00	
    Expon(6) = 4.29239273700000000E+01
    Coeffs(7) = 4.89091679300000000E+00	
    Expon(7) = 9.44929153800000000E+01
    Coeffs(8) = 4.18028750500000000E+00	
    Expon(8) = 2.08017103900000000E+02
    Coeffs(9) = 3.48295521000000000E+00	
    Expon(9) = 4.57929732900000000E+02
    Coeffs(10) = 1.77367055400000000E-01	
    Expon(10) = 1.00808845200000000E+03
    Coeffs(11) = 3.49730837400000000E+00	
    Expon(11) = 2.21921018600000000E+03
ELSE IF (Z_Atomic == 10) THEN
    Coeffs(1) = 8.00273577300000000E-02	
    Expon(1) = 5.17334281100000000E-01
    Coeffs(2) = 5.42362665100000000E-01	
    Expon(2) = 1.13575124700000000E+00
    Coeffs(3) = 2.90299108300000000E+00	
    Expon(3) = 2.49341855200000000E+00
    Coeffs(4) = 8.83031519500000000E-01	
    Expon(4) = 5.47402971700000000E+00
    Coeffs(5) = 8.39379158500000000E+00	
    Expon(5) = 2.63834192900000000E+01
    Coeffs(6) = 6.99336489000000000E+01	
    Expon(6) = 5.79219325600000000E+01
    Coeffs(7) = 1.04383394200000000E+02	
    Expon(7) = 1.27161314300000000E+02
    Coeffs(8) = 1.17391138800000000E+02	
    Expon(8) = 2.79168859400000000E+02
    Coeffs(9) = 9.38734165100000000E+01	
    Expon(9) = 6.12884921200000000E+02
    Coeffs(10) = 6.18959986800000000E+01	
    Expon(10) = 1.34552230300000000E+03
    Coeffs(11) = 6.90819873000000000E+01	
    Expon(11) = 2.95394813100000000E+03
    Coeffs(12) = 5.60011458600000000E+01	
    Expon(12) = 1.42372723800000000E+04
    Coeffs(13) = 5.53432860400000000E+00	
    Expon(13) = 3.12563857500000000E+04
ELSE IF (Z_Atomic == 12) THEN
    Coeffs(1) = 0.1056541802E-01    
    Expon(1) = 0.1119931034E+00
    Coeffs(2) = 0.1356519906E-01    
    Expon(2) = 0.2375019538E+00
    Coeffs(3) = 0.1174603920E+00    
    Expon(3) = 0.1068117428E+01
    Coeffs(4) = 0.2110141931E+01    
    Expon(4) = 0.2265139268E+01
    Coeffs(5) = 0.7403592664E+01    
    Expon(5) = 0.4803644023E+01
    Coeffs(6) = 0.1383699028E+00    
    Expon(6) = 0.1018700979E+02
    Coeffs(7) = 0.2788102492E+02    
    Expon(7) = 0.4581403450E+02
    Coeffs(8) = 0.1377995932E+03    
    Expon(8) = 0.9715707822E+02
    Coeffs(9) = 0.1770071493E+03    
    Expon(9) = 0.2060394364E+03
    Coeffs(10) = 0.2081874665E+03    
    Expon(10) = 0.4369444835E+03
    Coeffs(11) = 0.1309155764E+03    
    Expon(11) = 0.9266210634E+03
    Coeffs(12) = 0.1507477824E+03    
    Expon(12) = 0.1965070226E+04
    Coeffs(13) = 0.3952103802E+02    
    Expon(13) = 0.4167292486E+04
    Coeffs(14) = 0.1069116883E+03    
    Expon(14) = 0.8837509432E+04
    Coeffs(15) = 0.4291887253E+02    
    Expon(15) = 0.3974492714E+05
    Coeffs(16) = 0.3152951047E+02    
    Expon(16) = 0.8428642090E+05
ELSE IF (Z_Atomic == 18) THEN
    Coeffs(1) = 0.5473904910E-02    
    Expon(1) = 0.1442207120E+00
    Coeffs(2) = 0.6829939177E+00    
    Expon(2) = 0.6119681072E+00
    Coeffs(3) = 0.1177066792E+00    
    Expon(3) = 0.1260605990E+01
    Coeffs(4) = 0.1164532302E+00    
    Expon(4) = 0.5349097580E+01
    Coeffs(5) = 0.4416586452E+02    
    Expon(5) = 0.1101871874E+02
    Coeffs(6) = 0.1067763036E+02    
    Expon(6) = 0.2269769075E+02
    Coeffs(7) = 0.4037436211E+02    
    Expon(7) = 0.9631253829E+02
    Coeffs(8) = 0.4393893376E+03    
    Expon(8) = 0.1983962257E+03
    Coeffs(9) = 0.5563267702E+03    
    Expon(9) = 0.4086805628E+03
    Coeffs(10) = 0.7430814518E+03    
    Expon(10) = 0.8418496968E+03
    Coeffs(11) = 0.4609451848E+03    
    Expon(11) = 0.1734143917E+04
    Coeffs(12) = 0.5336556378E+03    
    Expon(12) = 0.3572199570E+04
    Coeffs(13) = 0.2203764729E+03    
    Expon(13) = 0.7358449114E+04
    Coeffs(14) = 0.2800829655E+03    
    Expon(14) = 0.1515782428E+05
    Coeffs(15) = 0.1771944629E+03    
    Expon(15) = 0.3122392140E+05
    Coeffs(16) = 0.2143058729E+03    
    Expon(16) = 0.1324916776E+06
    Coeffs(17) = 0.7643772860E+01    
    Expon(17) = 0.2729223965E+06
ELSE
   WRITE(*,*) 'The atom with atomic number ',Z_atomic,' is not in the set.'
   WRITE(*,*) 'The program will stop.'
   WRITE(101,*) 'The atom with atomic number ',Z_atomic,' is not in the set.'
   WRITE(101,*) 'The program will stop.'
END IF

END subroutine get_density_constants

subroutine 


end MODULE INOUT_MODULE





