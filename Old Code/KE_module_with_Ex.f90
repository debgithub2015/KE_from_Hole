...
! a(:) -- a generic variable equal to various "alpha" variables.                !
! spin -- a spin counter                                                        !
...
REAL(dbl)     :: localKE(1:n_grid),dij(1:n_grid,1:n_grid),pdist(1:n_grid,1:n_grid)
REAL(dbl)     :: a(1:n_grid),gUEG(1:n_grid,1:n_grid)
REAL(dbl)     :: hole_kernel(1:n_grid,1:n_grid,1:2),Ex_kernel(1:n_grid,1:n_grid,1:2)
INTEGER(istd) :: i,j,k,l      !just a counter
INTEGER(istd) :: refpoints,spin
LOGICAL :: debug
...
IF (debug) THEN
   !Print Normalization and Exchange-Energy Values
   !Compute distance between 3d grid points:
   ...
   DO refpoints = 0,2
      ...
      DO spin=1,2
         ...
         FORALL(k=1:n_grid,l=1:n_grid)
                hole_kernel(k,l,spin) = rho(k,spin)*rho(l,spin)*(-1)*gUEG(k,l)**2  &
                                        *Wbecke(k)*Wbecke(l)                
         ENDFORALL
         FORALL(k=1:n_grid,l=1:n_grid,(dij(k,l)>.0000001_dbl))
                Ex_kernel(k,l,spin) = hole_kernel(k,l,spin)/(2*dij(k,l))                
         ENDFORALL
      ENDDO
      WRITE(*,*) ' '
      WRITE(*,*) 'For normalization using the following number of points:',refpoints
      WRITE(*,*) 'The normalization test gives: ',-1*SUM(hole_kernel)
      WRITE(*,*) 'The exchange energy is: ',SUM(Ex_kernel)
      WRITE(*,*) ' '
   ENDDO
ENDIF
