...
f(:) = 0.0_dbl
FORALL(k=1:n_grid)
      f(k) = rho(k)*Wbecke(k) * (DOT_PRODUCT(Wbecke(:),rho(:)*(-1)*gUEG(k,:)**2) + 1)
ENDFORALL
...
FORALL(k=1:n_grid)
      ...
      !negligible error, since we won't ever converge the a's that accurately anyway.
         Jdiag(k) = -2*rho(k)*Wbecke(k)*DOT_PRODUCT(Wbecke(:),rho(:)*gUEG(k,:)*dgUEG(k,:)           &
                                        *(a(k)+EPSILON(a(k))**2)**(p_mean-1)                        &
                                        *pdist(k,:)/(a(k)**p_mean+a(:)**p_mean+EPSILON(a(k))**2))
ENDFORALL
...
subroutine normalization1ptunscaled(rho,a,f,Jdiag)
...
FORALL(k=1:n_grid)
      f(k) = (DOT_PRODUCT(Wbecke(:),rho(:)*(-1)*gUEG(k,:)**2) + 1)
      Jdiag(k) = (2*DOT_PRODUCT(Wbecke(:),rho(:)*(-1)*gUEG(k,:)*dij(k,:)*dgUEG(k,:)))
ENDFORALL
...
end subroutine normalization1ptunscaled
...
subroutine normalization2ptunscaled(rho,a,f)
...
FORALL(k=1:n_grid)
         f(k) = (DOT_PRODUCT(Wbecke(:),rho(:)*(-1.0_dbl)*gUEG(k,:)**2) + 1.0_dbl)
ENDFORALL

end subroutine normalization2ptunscaled
...
subroutine normalization2ptJacobianunscaled(rho,a,f,Jdiag)
...
end subroutine normalization2ptJacobianunscaled
